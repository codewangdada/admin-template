<template>
  <div class="tags-view-container" ref="tags">
    ...
    >
      ...
      >
        {{ tag.title }}
        <span v-if="!isAffix(tag)" @click.prevent.stop="closeSelectTag(tag)">
          <close
            class="el-icon-close"
    ...
    <ul
      ...
      class="contextmenu"
    >
      <li @click="refreshSelectTag(selectedTag)">
        <refresh-right style="width: 1em; height: 1em" />
        刷新页面
      </li>
      <li v-if="!isAffix(selectedTag)" @click="closeSelectTag(selectedTag)">
...
const visitedViews = computed(() => useTagsStore().visitedViews)
const routes = computed(() => useUserStore().menuRoutes)
...
function isFirstView() {
  try {
    return (
      ...
      selectedTag.value.fullPath === '/home'
    )
  } catch (error) {
    return false
  }
...
function closeSelectTag(view: any) {
...
function openMenu(tag, e) {
  ...
  const offsetLeft = tags.value.getBoundingClientRect().left // container margin left
  const offsetWidth = tags.value.offsetWidth // container width
...
function refreshSelectTag(view: any) {
    ...
    for (const r of visitedViews.value) {
      if (r.path === route.path) {
        ...
        if (r.fullPath !== route.fullPath) {
          useTagsStore().updateVisitedView(route)
        }
      }
    }
  })
...
.tags-view-container {
  ...
  box-shadow: 0 1px 3px 0 raba(0, 0, 0, 0.12), 0 0 3px 0 rgba(0, 0, 0, 0.04);
  .tags-view-wrapper {
    .tags-view-item {
